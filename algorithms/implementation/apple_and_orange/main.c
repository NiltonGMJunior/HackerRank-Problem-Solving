...
void countApplesAndOranges(int s, int t, int a, int b, int apples_count, int* apples, int oranges_count, int* oranges) {
    /*  s               -   Starting x coordinate of Sam's house
        t               -   Ending x coordinate of Sam's house
        a               -   x coordinate of the apple tree
        b               -   x coordinate of the orange tree
        apple_count     -   number of apples that fell from the tree
        apples          -   pointer to the vector of distances from a along the x axis
        oranges_count   -   number of orangfes that fell from the tree
        oranges         -   pointer to the vector of distances from b along the x axis 
     */

    // int *apples_coordinates = (int *)(malloc(apples_count * sizeof(int *)));
    // if (!apples_coordinates)
    // {
    //     printf("Failed to allocate memory. Exiting program.\n");
    //     exit(-1);
    // }

    // int *oranges_coordinates = (int *)(malloc(oranges_count * sizeof(int *)));
    // if (!oranges_coordinates)
    // {
    //     printf("Failed to allocate memory. Exiting program.\n");
    //     exit(-1);
    // }

    // memcpy(apples_coordinates, apples, apples_count * sizeof(*apples));
    // memcpy(oranges_coordinates, apples, oranges_count * sizeof(*oranges));
    ...
    printf("%d\n%d\n", apples_in_house, oranges_in_house);
